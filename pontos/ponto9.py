...
# Função para criar a pasta do vigia se não existir
...
def capturar_imagem_webcam(vigia_nome):
    ...
    if ret:
        ...
        nome_imagem = f'{vigia_nome}_{data_hora_atual}.jpg'

        cv2.imwrite(os.path.join(vigia_nome, nome_imagem), frame)
...
def registrar_ponto(vigia_nome, imagem_path):
    ...
    if hora_formatada:
        ...
        if os.path.exists(os.path.join(vigia_nome, f'{vigia_nome}_registro_ponto.xlsx')):
            dados_antigos = pd.read_excel(os.path.join(vigia_nome, f'{vigia_nome}_registro_ponto.xlsx'))
            dados_completos = pd.concat([dados_antigos, dados_novos], ignore_index=True)
        else:
            dados_completos = dados_novos
        
        dados_completos.to_excel(os.path.join(vigia_nome, f'{vigia_nome}_registro_ponto.xlsx'), index=False)
...
janela.after(0, atualizar_relogio)
janela.mainloop()
